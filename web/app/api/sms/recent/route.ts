...
// 서비스 선택 시 "최근 수신 성공한 국가" (참고용). 회원 전체 기준 최근 7일.
export async function GET(req: Request) {
  ...
  const countries: { value: string; label: string; iso: string; at: Date }[] = [];
  for (const r of rows) {
    if (seen.has(r.country)) continue;
    const c = COUNTRIES.find((x) => x.value === r.country);
    if (!c) continue; // 목록에서 빠진 국가는 선택할 수 없으므로 제외
    seen.add(r.country);
    countries.push({ value: c.value, label: c.label, iso: c.iso, at: r.createdAt });
