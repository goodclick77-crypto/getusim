...
/**
 * 충전 화면의 횟수 단위(클릭하면 누적). 3회를 두 번 누르면 6회.
 * 화면은 "인증 N회"로 보여주고 내부 포인트 = 횟수 × SMS_BASE_POINT 로 환산한다.
 * 금액으로 보여줄 때 생기던 "둥근 금액 충전 → 잔여 포인트 환불" 패턴을 줄이기 위한 것.
 */
export const CHARGE_COUNT_UNITS = [1, 3, 5, 10, 50, 100];
